...
### Character merge/split UI + per-alias confidence (Phase 4.4)

- **New `CharacterAlias` Prisma side table replaces the legacy JSON column.** Migration `20260724000000_add_character_alias_confidence` creates `CharacterAlias (id, characterId, alias, confidence Float, source String, detectedInChapter Int?, createdAt, updatedAt)` with `@@unique([characterId, alias])` and `@@index([source, confidence])`. Existing JSON aliases are backfilled via `json_each(c."aliases")` with `confidence=1.0, source='legacy'`, then the JSON column is dropped in the same migration. UUIDs are synthesised in SQL via `lower(hex(randomblob(...)))` since SQLite has no native UUID type. The new table is the single source of truth; the read path (`listCharacters`) aggregates aliases into both `aliases: string[]` (for legacy consumers) and `aliasDetails: {id, alias, confidence, source, detectedInChapter}[]` (for the new UI).
- **Per-alias confidence scoring.** New `src/lib/ai/character-alias-confidence.ts` exports `computeAliasConfidence(primaryName, alias, signals)` with bases: `exact/normalized → 0.95`, `substring → 0.85`, `levenshtein → 0.75`, `llm → 0.6`. Modifiers: sample-lines bonus capped at +0.20, crowding decay ×0.85 per alias beyond the third. Self-alias short-circuits to 1.0 (bypasses bonus + decay since "this character has many aliases" doesn't diminish the fact that the canonical name is unambiguous). Output is clamped to [0, 1] and rounded to 2dp. Threshold constants `LOW_CONFIDENCE_THRESHOLD=0.6`, `HIGH_CONFIDENCE_THRESHOLD=0.8` drive the badge tiers via `classifyAliasScore()` → `'high' | 'medium' | 'low'`.
- **Three new API routes** (all invalidate the audiobook cache via `setBookAudiobookStatus(..., 'none')` after a successful write):
  - `POST /api/library/[id]/characters/merge` — body `{survivorId, absorbedId, aliasResolutions?: [{alias, keepOn: 'survivor'|'absorbed'}]}`. Aliases fold with highest-confidence wins per shared alias (caller override via `aliasResolutions`); appearances get summed; `CharacterRelationship` edges rewired; `CharacterProfile` absorbed if survivor has none; absorbed row deleted (cascades aliases). Returns 400 on self-merge, 404 on missing character, 409 on user-locked profile-field conflict.
  - `POST /api/library/[id]/characters/split` — body `{characterId, aliasesToMove[], newName, newRole?, newVoiceName?}`. New `Character` row is created, named aliases move to it with `source='user'` (a manual split is a user-authoritative action). Returns 400 on empty `aliasesToMove`, 404 on missing source / non-belonging aliases, 409 on name collision. Optional `newVoiceName` (must be a built-in VieNeu voice) creates a `Voice` row + assigns `voiceId` to the new character. Response includes `caveat: 'Chapter appearances are NOT moved by split; this is a roster change. Re-run detection if you want a fresh chapter assignment.'` so callers know to re-detect.
  - `PATCH /api/library/[id]/characters/[characterId]/aliases/[aliasId]` — body `{confidence?, source?, alias?}`. Used by the Aliases tab to mark a low-confidence alias as wrong (`confidence=0, source='user'`) or to rename / re-tag a single alias. Returns 404 if alias doesn't belong to the character / book.
- **New `CharacterMergeSplitPanel.tsx`** mounted on `/library/[id]/page.tsx` below `<WatermarksPanel />`. Card header carries the "needs review" badge summing `pendingCount + lowConfidenceCount` (sourced from `/api/library/${bookId}/characters/bible` + the character list). Three tabs driven by the existing `<Tabs>` primitive:
  - **Gộp (Merge)** — two `<CharacterSelect>` dropdowns (Giữ lại / Hấp thụ), side-by-side `<CharacterSummary>`, per-shared-alias radio defaulting to higher-confidence, `<Dialog>` confirm wrapping `toast.confirm({destructive: true})`.
  ...
  All three tabs share a `refetchAll()` that re-pulls the character list + bible endpoint so badges update after each write. Loading + empty + error states mirror `WatermarksPanel.tsx`.
- **Discriminated `CharacterMutationResult<T>` helper return type.** New `src/lib/db/characters.ts` exports `{ ok: true, data: T } | { ok: false, error: CharacterMutationError }` for both `mergeCharacters()` and `splitCharacter()`. Error kinds: `'survivor-not-found' | 'absorbed-not-found' | 'cross-book' | 'self-merge' | 'profile-conflict' | 'empty-aliases' | 'name-collision'`. The route handlers switch on `result.error.kind` to map to 400/404/409. `patchCharacterAlias()` returns the same shape with `survivor-not-found` and `cross-book` variants.
- **Write-through updates.** `src/lib/db/voices.ts` `upsertCharacters` wraps each upsert in `prisma.$transaction(async tx => …)` and syncs `CharacterAlias` rows via `findUnique + create/update` keyed on `characterId_alias` instead of JSON-stringifying into the deleted column. `src/app/api/library/[id]/characters/detect/route.ts` writes per-alias rows with `computeAliasConfidence(...)` + `foldMethod='normalized' | 'llm'` instead of producing a JSON blob. `src/lib/db/character-bible.ts` `ensureCharacter` reads aliases via `include: { aliases: true }`, `resolveCharacterIds` selects aliases via `c.aliases.map(a => a.alias)`, and `mergeAliasLists` now returns `string[]` (deduped subset to add) instead of `{value, changed}`.
...
- `src/tests/m4b-export.test.ts` — 15 new unit tests covering: `buildFfMetadata` empty-chapters throw, single chapter block (magic header + TIMEBASE), three-chapter cumulative START/END math, **strict chapter boundary ordering** (ch N's END == ch N+1's START — catches off-by-one regressions that looser assertions miss), fractional-duration rounding + non-positive/NaN throw, UTF-8 Vietnamese + special-character escaping, `exportM4B` arg set with cover (verifies input index 2 + `-map 2:v -attached_pic`), arg set without cover (exactly 2 `-i` flags, no `2:v`), `proc.on('error') ENOENT` graceful degradation, **`ESAFEPATH` thrown when cover path escapes uploads/library roots** (security gate, not silent-strip), silent cover-strip when cover is in-root but file is missing (audio still exports), chapter-file-missing throws `EUNKNOWN` rather than masquerading as `ESAFEPATH`. `getActualDurations()` drift-correction helper now has 3 dedicated tests: parses ffprobe seconds → ms, returns 0 on unparseable output, returns 0 on spawn error (best-effort contract).
...
**Test totals:** **278/278 JS** tests pass (was 263/263 after Phase 4.5 initial; +11 route-level tests at `d414d577`, +4 review-fixes). `tsc --noEmit` clean. Python tests untouched.
...
- **Cover ESAFEPATH no longer silently swallowed.** `validateInputs` previously caught *all* `assertWithinRoots` errors for the cover and cleared `coverPath` — masking the same security event the audio-path branch surfaces as `ESAFEPATH`. Now the contract is split: cover path outside uploads/library roots → `ESAFEPATH` thrown synchronously (route surfaces as 500, no detail leak); cover path in-root but file missing → silent-strip (audio still exports). This aligns the cover path with the audio-path security posture — a malicious DB row pointing at `/etc/passwd` via `coverPath` is now rejected the same way it would be via `audioPath`.
